...
describe("Base API - Query Functions", () => {
  ...
  describe("query", () => {
    ...
    it("should throw NotFoundError when throwOnEmpty is true and no data", async () => {
      ...
      await expect(query("test_table", { throwOnEmpty: true })).rejects.toThrow(
        "No records found",
      );
    });
...
describe("Base API - CRUD Operations", () => {
  ...
  describe("insert", () => {
    it("should insert single record", async () => {
      ...
      const result = await insert<{ id: string; name: string }>("test_table", {
        name: "New Item",
      });
    ...
    it("should throw error when insert fails", async () => {
      ...
      await expect(insert("test_table", { name: "New Item" })).rejects.toThrow(
        "Insert failed",
      );
    });
  });
...
describe("Base API - Integration", () => {
  ...
  it("should handle complete CRUD workflow", async () => {
    ...
    const inserted = await insert<{ id: string; name: string }>("test_table", {
      name: "Test",
    });
