...
// Create mocks at module scope so they can be reconfigured in tests
const { mockInitialize, mockIsReady } = vi.hoisted(() => ({
  mockInitialize: vi.fn(() => Promise.resolve(undefined)),
...
describe("OfflineProvider", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // No need to mock again, already setup in vi.mock
  });
  ...
  describe("Error Handling", () => {
    it("should handle IndexedDB initialization error gracefully", async () => {
      const consoleErrorSpy = vi
        .spyOn(console, "error")
        .mockImplementation(() => {});
      mockInitialize.mockRejectedValue(new Error("IndexedDB not supported"));
