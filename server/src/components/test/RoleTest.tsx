    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        ...
        <div className="space-y-2">
          ...
          <p>
            <strong>Role:</strong>{" "}
            <span className={`text-${roleColor}-600 font-semibold`}>
              {roleLabel}
            </span>
          </p>
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex items-center gap-2">
            <span
              className={`w-3 h-3 rounded-full ${isAdmin ? "bg-green-500" : "bg-gray-300"}`}
            />
            <span>isAdmin: {isAdmin ? "✅" : "❌"}</span>
          </div>
          <div className="flex items-center gap-2">
            <span
              className={`w-3 h-3 rounded-full ${isDosen ? "bg-green-500" : "bg-gray-300"}`}
            />
            <span>isDosen: {isDosen ? "✅" : "❌"}</span>
          </div>
          <div className="flex items-center gap-2">
            <span
              className={`w-3 h-3 rounded-full ${isMahasiswa ? "bg-green-500" : "bg-gray-300"}`}
            />
            <span>isMahasiswa: {isMahasiswa ? "✅" : "❌"}</span>
          </div>
          <div className="flex items-center gap-2">
            <span
              className={`w-3 h-3 rounded-full ${isLaboran ? "bg-green-500" : "bg-gray-300"}`}
            />
      ...
      <Card className="p-6">
        ...
        <div className="space-y-2">
          <PermissionRow label="View Kuis" result={canView("kuis")} />
          <PermissionRow label="Create Kuis" result={canCreate("kuis")} />
          <PermissionRow label="Manage Kuis" result={canManage("kuis")} />
          <PermissionRow
            label="Can create:mata_kuliah"
            result={can("create", "mata_kuliah")}
          />
          <PermissionRow label="Can view:nilai" result={can("view", "nilai")} />
          <PermissionRow
          ...
          {isMahasiswa && (
            <div className="p-3 bg-green-50 border border-green-200 rounded">
              ...
              <p className="text-sm text-green-600">
                You can take quizzes and view grades
              </p>
            </div>
          )}
...
function PermissionRow({ label, result }: { label: string; result: boolean }) {
  return (
    <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
      ...
      <span
        className={`font-semibold ${result ? "text-green-600" : "text-red-600"}`}
      >
