...
async function getDashboardStatsImpl(): Promise<DashboardStats> {
  try {
    ...
    const totalMahasiswa =
      users?.filter((u) => u.role === "mahasiswa").length || 0;
...
export const getDashboardStats = requirePermission(
  "view:dashboard",
...
// 🔒 PROTECTED: Requires view:dashboard permission
export const getUserGrowth = requirePermission(
  "view:dashboard",
  getUserGrowthImpl,
);
...
// 🔒 PROTECTED: Requires view:dashboard permission
export const getUserDistribution = requirePermission(
  "view:dashboard",
  getUserDistributionImpl,
);
...
// 🔒 PROTECTED: Requires view:dashboard permission
export const getRecentUsers = requirePermission(
  "view:dashboard",
  getRecentUsersImpl,
);
...
async function getRecentAnnouncementsImpl(
  limit: number = 5,
  ...
  try {
    ...
    return (
      (data as PengumumanData[] | null)?.map((item) => ({
        id: item.id,
        title: item.judul,
...
// 🔒 PROTECTED: Requires view:dashboard permission
export const getRecentAnnouncements = requirePermission(
  "view:dashboard",
  getRecentAnnouncementsImpl,
);
