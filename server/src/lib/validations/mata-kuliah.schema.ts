...
const baseMataKuliahSchema = z.object({
  kode_mk: z
    ...
    .refine((val) => isValidKodeMK(val), "Invalid Kode MK format"),
  ...
  nama_mk: z
    ...
    .min(
      10,
      "Nama mata kuliah minimal 10 karakter untuk nama yang jelas dan profesional",
    )
  ...
  sks: z
    ...
    .refine((val) => isValidSKS(val), "SKS must be between 1 and 6"),
  ...
  semester: z
    ...
    .refine((val) => isValidSemester(val), "Semester must be between 1 and 14"),

  program_studi: z.enum(PROGRAM_STUDI_OPTIONS as any, {
...
export const mataKuliahFilterSchema = z.object({
  ...
  program_studi: z.enum([...PROGRAM_STUDI_OPTIONS] as any).optional(),
...
export const assignMahasiswaSchema = z.object({
  mata_kuliah_id: z.string().uuid("Invalid mata kuliah ID"),
  ...
  kelas_id: z.string().uuid("Invalid kelas ID").optional(),
});
...
export type BulkDeleteMataKuliahFormData = z.infer<
  typeof bulkDeleteMataKuliahSchema
>;
 ...
 */
export function parseCreateMataKuliahForm(
  data: unknown,
): CreateMataKuliahFormData {
  return createMataKuliahSchema.parse(data);
}

export function parseUpdateMataKuliahForm(
  data: unknown,
): UpdateMataKuliahFormData {
  return updateMataKuliahSchema.parse(data);
}

export function parseMataKuliahFilters(
  data: unknown,
): MataKuliahFilterFormData {
