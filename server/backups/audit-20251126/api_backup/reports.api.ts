...
export async function getEquipmentStats(): Promise<EquipmentStats> {
  try {
    ...
    const stats: EquipmentStats = {
      ...
      low_stock: data?.filter((i) => i.jumlah_tersedia < 5 && i.jumlah_tersedia > 0).length || 0,
      ...
      total_categories: new Set(data?.map((i) => i.kategori).filter(Boolean)).size,
    };
...
export async function getLabUsageStats(): Promise<LabUsageStats> {
  try {
    ...
    const stats: LabUsageStats = {
      ...
      total_capacity: labsResult.data?.reduce((sum, lab) => sum + (lab.kapasitas || 0), 0) || 0,
    };
 ...
 */
export async function getTopBorrowedItems(limit: number = 10): Promise<TopBorrowedItem[]> {
  try {
    const { data, error } = await supabase
      .from('peminjaman')
      .select(`
    ...
    const aggregated = (data || []).reduce((acc: Record<string, TopBorrowedItem>, item: Record<string, unknown>) => {
 ...
 */
export async function getBorrowingTrends(days: number = 30): Promise<BorrowingTrend[]> {
  try {
    ...
    // Group by date
    const grouped = (data || []).reduce((acc: Record<string, BorrowingTrend>, item) => {
      const date = new Date(item.created_at || item.tanggal_pinjam).toISOString().split('T')[0];
      ...
      acc[date].count += 1;
      if (item.status === 'approved' || item.status === 'returned') acc[date].approved += 1;
...
export async function getLabUtilization(): Promise<LabUtilization[]> {
  try {
    ...
    const aggregated = (data || []).reduce((acc: Record<string, LabUtilization>, item: Record<string, unknown>) => {
...
export async function getRecentActivities(limit: number = 20): Promise<RecentActivity[]> {
  try {
    ...
    return ((data as any) || []).map((item: Record<string, unknown>) => {
      const peminjam = item.peminjam as Record<string, unknown>;
      const user = peminjam?.user as Record<string, unknown>;
      const inventaris = item.inventaris as Record<string, unknown>;
      const userName = (user?.full_name as string) || 'Unknown';
      const equipmentName = (inventaris?.nama_barang as string) || 'Unknown';

      let type: 'borrowing' | 'return' | 'approval' | 'rejection' = 'borrowing';
