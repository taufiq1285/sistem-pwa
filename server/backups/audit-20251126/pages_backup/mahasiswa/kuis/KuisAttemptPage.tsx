...
export default function KuisAttemptPage() {
  ...
  const validateQuizAccess = async () => {
    ...
    try {
      ...
      const isActive = (quizData as any).is_active ?? (quizData as any).status === 'active';
      if (!isActive) {
        setError('Kuis ini belum aktif atau sudah ditutup');
        setCanAttempt(false);
        return;
      }
      ...
      // Check timing
      const now = new Date();
      const startDate = new Date(quizData.tanggal_mulai);
      const endDate = new Date(quizData.tanggal_selesai);
      
      if (now < startDate) {
        setError(`Kuis ini akan dimulai pada ${startDate.toLocaleString('id-ID')}`);
        setCanAttempt(false);
        return;
      }
      
      if (now > endDate) {
        setError('Waktu pengerjaan kuis telah berakhir');
        setCanAttempt(false);
        return;
      }
