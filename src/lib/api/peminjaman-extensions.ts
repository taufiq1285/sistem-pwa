...
export async function getAllPeminjaman(params?: {
  ...
  try {
    ...
    const mappedData = ((data ?? []) as unknown as PeminjamanQueryRow[]).map(
      (item) => ({
        id: item.id,
 ...
 */
export async function getPendingRoomBookings(
  limit: number = 50,
): Promise<RoomBookingRequest[]> {
  try {
    // Step 1: Get jadwal_praktikum data
    const { data, error } = await supabase
      .from("jadwal_praktikum")
      .select(
        "id, kelas_id, laboratorium_id, hari, jam_mulai, jam_selesai, tanggal_praktikum, minggu_ke, topik, deskripsi, catatan, created_at",
      )
      .eq("is_active", false)
    ...
    const labMap = new Map(
      (labData.data || []).map((lab: any) => [lab.id, lab]),
    );
    ...
    const dosenMap = new Map(
      (dosenData.data || []).map((dosen: any) => [dosen.id, dosen]),
    );
    const kelasMap = new Map(
      (kelasData.data || []).map((kelas: any) => [kelas.id, kelas]),
    );
 ...
 */
export async function rejectRoomBooking(
  jadwalId: string,
  reason?: string,
): Promise<void> {
  try {
