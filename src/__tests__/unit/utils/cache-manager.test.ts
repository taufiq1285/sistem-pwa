...
describe('Cache Manager', () => {
  ...
  describe('initializeCacheManager', () => {
    ...
    it('should preserve auth data on version change', () => {
      ...
      localStorage.setItem('sb-lqkzhrdhrbexdtrgmogd-auth-token', 'supabase_token');
  ...
  describe('debugStorage', () => {
    it('should log storage information to console', () => {
      ...
      expect(localStorageCall?.[1]).toEqual(expect.arrayContaining(['app_version', 'data1']));
      ...
      expect(sessionStorageCall?.[1]).toEqual(expect.arrayContaining(['session1']));
