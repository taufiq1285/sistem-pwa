...
describe('Jadwal API', () => {
  ...
  describe('getCalendarEvents', () => {
    ...
    it('should skip jadwal with null tanggal_praktikum', async () => {
      ...
      const jadwalWithoutDate = { ...mockJadwalWithLab, tanggal_praktikum: null };
      vi.mocked(baseApi.queryWithFilters).mockResolvedValue([jadwalWithoutDate]);
    ...
    it('should handle invalid time parsing', async () => {
      ...
      vi.mocked(baseApi.queryWithFilters).mockResolvedValue([jadwalWithInvalidTime]);
  ...
  describe('checkJadwalConflict (Legacy)', () => {
    it('should check conflicts by hari', async () => {
      ...
      const result = await checkJadwalConflict('lab-1', 'senin', '08:00', '10:00');
  ...
  describe('getJadwalPraktikumMahasiswa', () => {
    ...
    it('should fetch jadwal with date range', async () => {
      ...
      const startDate = new Date('2025-01-01');
      const endDate = new Date('2025-01-31');
      const result = await getJadwalPraktikumMahasiswa('user-1', startDate, endDate);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle jadwal without laboratorium relation', async () => {
      const jadwalWithoutLab = { ...mockJadwal, laboratorium: undefined };
      ...
      expect(result[0].title).toBe('Kelas A - Lab');

      consoleSpy.mockRestore();
    });

    it('should handle multiple jadwal on same date', async () => {
      const jadwal1 = { ...mockJadwal, id: 'jadwal-1', jam_mulai: '08:00', jam_selesai: '10:00' };
      const jadwal2 = { ...mockJadwal, id: 'jadwal-2', jam_mulai: '10:00', jam_selesai: '12:00' };
