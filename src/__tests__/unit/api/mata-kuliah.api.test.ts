...
import { queryWithFilters, query, getById, insert, update, remove, count } from '../../../lib/api/base.api';
...
describe('Mata Kuliah API', () => {
  ...
  describe('deleteMataKuliah', () => {
    ...
    it('should reject when kelas exists', async () => {
      vi.mocked(count).mockResolvedValue(5);

      await expect(deleteMataKuliah('mk-1')).rejects.toThrow('active kelas');
    });
  });
