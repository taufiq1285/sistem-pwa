...
const mockDosenData = [
  { user_id: 'user-2', nip: '198501012010121001', nidn: '0001018501', phone: '081234567891' },
];
...
describe('Users API - Statistics', () => {
  ...
  describe('getUserStats', () => {
    it('should calculate user statistics correctly', async () => {
      ...
      const mahasiswaBuilder = mockQueryBuilder();
      mahasiswaBuilder.in.mockResolvedValue({ data: mockMahasiswaData, error: null });
...
describe('Users API - CRUD Operations', () => {
  ...
  describe('deleteUser - CASCADE DELETE', () => {
    it('should delete mahasiswa from role table then users table', async () => {
      ...
      deleteUserBuilder.eq.mockResolvedValue({ error: null });
    ...
    it('should continue even if role-specific delete fails', async () => {
      const getUserBuilder = mockQueryBuilder();
      getUserBuilder.single.mockResolvedValue({
        data: { role: 'mahasiswa' },
        error: null,
      ...
      // Should not throw, continues to delete from users table
      await deleteUser('user-3');
