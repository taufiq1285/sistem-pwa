...
describe("QueueManager", () => {
  ...
  describe("Initialization", () => {
    ...
    it("should not re-initialize if already initialized", async () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        "QueueManager already initialized",
      );
    });
  ...
  describe("Enqueue", () => {
    ...
    it("should enqueue an item", async () => {
      const item = await queueManager.enqueue("kuis", "create", {
        name: "Test Quiz",
      });
    ...
    it("should store item in IndexedDB", async () => {
      const item = await queueManager.enqueue("materi", "update", {
        id: "123",
      });

      const stored = await indexedDBManager.read<SyncQueueItem>(
        "sync_queue",
        item.id,
      );
  ...
  describe("Process Queue", () => {
    ...
    it("should mark items as syncing during processing", async () => {
      ...
      const processor = vi
        .fn()
        .mockImplementation(async (item: SyncQueueItem) => {
          // Check status during processing
          const current = await indexedDBManager.read<SyncQueueItem>(
            "sync_queue",
            item.id,
          );
