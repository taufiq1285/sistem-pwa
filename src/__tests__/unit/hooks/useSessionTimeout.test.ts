...
describe("useSessionTimeout Hook", () => {
  ...
  describe("Timer Coverage - setTimeout/clearTimeout", () => {
    ...
    it("should clear existing timers before setting new ones (resetTimeout)", () => {
      ...
      // Initial mount: 2 timers set
      expect(setTimeoutSpy).toHaveBeenCalledTimes(2);
      expect(clearTimeoutSpy).toHaveBeenCalledTimes(2); // Clear any existing timers

      // Rerender with different timeout
      rerender();

      // Should clear both old timers and set new ones
      // 2 clears from cleanup + 2 clears from resetTimeout = 4 clears
  ...
  describe("Branch Coverage - resetTimeout Function", () => {
    it("should branch: if (timeoutRef.current) clearTimeout - existing timeout", () => {
      ...
      // Trigger resetTimeout by rerendering
      rerender();
  ...
  describe("Error Handling", () => {
    ...
    it("should handle toast functions throwing errors", async () => {
      const mockErrorToast = {
        warning: vi.fn().mockImplementation(() => {
      ...
      vi.mocked((await import("sonner")).toast) = mockErrorToast as any;
      ...
      expect(mockErrorToast.warning).toHaveBeenCalled();
    });
  });
