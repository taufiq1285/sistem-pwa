...
describe('Permission Middleware', () => {
  ...
  describe('getCurrentUserWithRole', () => {
    ...
    it('should throw AuthenticationError if user not authenticated', async () => {
      ...
      await expect(getCurrentUserWithRole()).rejects.toThrow(AuthenticationError);
      await expect(getCurrentUserWithRole()).rejects.toThrow('User not authenticated');
    });
  ...
  describe('checkPermission', () => {
    ...
    it('should throw PermissionError if user lacks permission', async () => {
      ...
      await expect(checkPermission('create:kuis')).rejects.toThrow(PermissionError);
      await expect(checkPermission('create:kuis')).rejects.toThrow('Missing permission: create:kuis');
    });
  ...
  describe('requirePermission', () => {
    it('should execute function if permission granted', async () => {
      ...
      const mockFn = vi.fn().mockResolvedValue({ id: 'kuis-123', judul: 'Test Kuis' });
    ...
    it('should not execute function if permission denied', async () => {
      ...
      await expect(wrappedFn({ judul: 'Test' })).rejects.toThrow(PermissionError);
