...
export default function ManajemenAssignmentPage() {
  ...
  const [deleteConfirmation, setDeleteConfirmation] = useState<DeleteConfirmationData | null>(null);
  ...
  const fetchAssignments = async () => {
    try {
      ...
      // Calculate stats
      const totalJadwal = assignmentsWithSchedules?.reduce((sum: number, assignment: AssignmentDetail) =>
        sum + (assignment.jadwalDetail?.length || 0), 0) || 0;

      const uniqueDosenCount = new Set(assignmentsWithSchedules?.map((a: AssignmentDetail) => a.dosen_id)).size;

      setStats({
        totalAssignments: assignmentsWithSchedules?.length || 0,
        totalJadwal,
        activeAssignments: assignmentsWithSchedules?.filter((a: AssignmentDetail) =>
          a.jadwalDetail?.some((j: Jadwal) => j.status === 'scheduled')).length || 0,
  ...
  const confirmDeleteAssignment = async () => {
    ...
    try {
      const response = await fetch('/api/admin/delete-assignment-cascade', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          dosen_id: deleteConfirmation.assignment.dosen_id,
          mata_kuliah_id: deleteConfirmation.assignment.mata_kuliah_id,
          kelas_id: deleteConfirmation.assignment.kelas_id,
      ...
      <div className="space-y-4">
        {filteredAssignments.length === 0 ? (
          <Card>
            <CardContent className="pt-6">
              <div className="text-center text-muted-foreground">
                ...
                <p className="text-lg font-medium">Tidak ada assignment ditemukan</p>
          ...
          filteredAssignments.map((assignment) => (
            <Card key={`${assignment.dosen_id}-${assignment.mata_kuliah_id}-${assignment.kelas_id}`}>
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <Users className="h-5 w-5 text-blue-600" />
                      <span className="font-semibold">{assignment.dosen.full_name}</span>
                    ...
                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      <div className="flex items-center gap-1">
                        ...
                        <span>{assignment.mata_kuliah.kode_mk} - {assignment.mata_kuliah.nama_mk}</span>
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {assignment.tanggal_mulai && assignment.tanggal_selesai && (
                        <Badge variant="outline">
                          {format(new Date(assignment.tanggal_mulai), "dd MMM", { locale: localeId })} - {format(new Date(assignment.tanggal_selesai), "dd MMM yyyy", { locale: localeId })}
                        </Badge>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => toggleExpanded(`${assignment.dosen_id}-${assignment.mata_kuliah_id}-${assignment.kelas_id}`)}
                    >
                      {expandedRows.has(`${assignment.dosen_id}-${assignment.mata_kuliah_id}-${assignment.kelas_id}`) ? (
              ...
              {expandedRows.has(`${assignment.dosen_id}-${assignment.mata_kuliah_id}-${assignment.kelas_id}`) && (
                <CardContent className="border-t">
                  <div className="space-y-3">
                    <h4 className="text-sm font-semibold text-muted-foreground">Jadwal Praktikum Detail</h4>

                    {(!assignment.jadwalDetail || assignment.jadwalDetail.length === 0) ? (
                      <Alert>
                            ...
                            <div className="flex items-center gap-4">
                              <div className="text-sm">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span>{format(new Date(jadwal.tanggal_praktikum), "dd MMM yyyy", { locale: localeId })}</span>
                              ...
                              <div className="text-sm">
                                ...
                                {jadwal.topik && (
                                  <div className="text-muted-foreground">{jadwal.topik}</div>
                                )}
                              </div>
                            </div>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Tindakan ini akan menghapus assignment dan semua data terkait. Tindakan tidak dapat dibatalkan.
            </DialogDescription>
          </DialogHeader>
              ...
              <Alert className="border-red-200 bg-red-50">
                ...
                <AlertDescription>
                  ...
                  <ul className="mt-2 list-disc list-inside text-sm">
                    <li>Assignment untuk <strong>{deleteConfirmation.assignment.dosen.full_name}</strong></li>
                    <li>Mata kuliah <strong>{deleteConfirmation.assignment.mata_kuliah.nama_mk}</strong></li>
                    <li>Kelas <strong>{deleteConfirmation.assignment.kelas.nama_kelas}</strong></li>
                    <li>{deleteConfirmation.totalJadwal} jadwal praktikum terkait</li>
                  </ul>
                </AlertDescription>
              </Alert>
              ...
              <div className="space-y-3">
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setDeleteOptions(prev => ({ ...prev, alsoDeleteKelas: checked as boolean }))
                    }
                  />
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setDeleteOptions(prev => ({ ...prev, notifyDosen: checked as boolean }))
                    }
                  />
