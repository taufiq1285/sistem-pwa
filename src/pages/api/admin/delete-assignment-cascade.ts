...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!uuidRegex.test(dosen_id) || !uuidRegex.test(mata_kuliah_id) || !uuidRegex.test(kelas_id)) {
      return NextResponse.json(
        {
    ...
    console.log('🗑️ Deleting assignment cascade:', { dosen_id, mata_kuliah_id, kelas_id, options: validOptions });
