...
import { CheckCircle, XCircle, AlertCircle, Loader2, Clock, Package } from 'lucide-react';
...
import { getPendingApprovals, approvePeminjaman, rejectPeminjaman } from '@/lib/api/laboran.api';
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Requests yang belum diproses</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Catatan Penting</CardTitle>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">Tidak ada permintaan peminjaman menunggu</p>
            </div>
          ) : (
                ...
                <TableBody>
                  {requests.map((request) => (
                    <TableRow key={request.id}>
                      <TableCell>
                        <div className="font-medium text-sm">{request.peminjam_nama}</div>
                        <div className="text-xs text-muted-foreground">{request.peminjam_nim}</div>
                      </TableCell>
                      <TableCell>
                        <div className="font-medium text-sm">{request.inventaris_nama}</div>
                        <div className="text-xs text-muted-foreground">{request.inventaris_kode}</div>
                      </TableCell>
                      <TableCell className="text-sm">{request.laboratorium_nama}</TableCell>
                      <TableCell className="font-semibold">{request.jumlah_pinjam}</TableCell>
                      <TableCell className="text-sm">{formatDate(request.tanggal_pinjam)}</TableCell>
                      <TableCell className="text-sm">
                        {formatDate(request.tanggal_kembali_rencana)}
                      </TableCell>
                      <TableCell className="text-sm max-w-xs truncate">{request.keperluan}</TableCell>
                      <TableCell>
